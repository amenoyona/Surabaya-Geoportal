// Inisialisasi peta dengan fokus pada Keputih, Surabaya (Koordinat diperbarui)
const map = L.map('map').setView([-7.2945, 112.8005], 15);
...
function generateSampleData() {
    const centerLat = -7.2945;
    const centerLng = 112.8005;
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const line = L.polyline([start, end], {
            color: '#2ECC71', // Modern green (matching CSS)
            weight: 4,
            opacity: 0.7,
            dashArray: '8, 8'
        }).bindPopup(`
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const colors = { 'Ringan': '#F39C12', 'Sedang': '#E67E22', 'Berat': '#C0392B' }; // Matching CSS
    ...
    for (let i = 0; i < 15; i++) {
        const point = [
            centerLat + (Math.random() - 0.5) * 0.012,
            centerLng + (Math.random() - 0.5) * 0.012
        ];
        ...
        const marker = L.marker(point, {
            icon: L.divIcon({
                ...
                iconSize: [38, 38],
                className: 'custom-div-icon' // Ensure this class exists in CSS
            })
        }).bindPopup(`
...
// Feature click events for info display
